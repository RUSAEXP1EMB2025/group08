...
function doPost(e) {
  ...
  if (userMessage.toLowerCase() == "照度") {
    var spreadsheetId = "1hNZIP-bkVbVlCUdU8IKTJGDYV72XItLdRtlpuA6eF1k";
    var sheet = SpreadsheetApp.openById(spreadsheetId).getSheetByName("照度");
    sheet.appendRow([new Date(),Date.now(),getIlmi()]);
    return ContentService.createTextOutput(JSON.stringify({
      'content': 'Received message: ' + userMessage
    })).setMimeType(ContentService.MimeType.JSON);
  }
  ...
  return ContentService.createTextOutput(JSON.stringify({
    'content': 'Invalid message received: ' + userMessage
  })).setMimeType(ContentService.MimeType.JSON);
}
